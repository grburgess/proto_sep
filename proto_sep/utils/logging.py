...
proto_sep_dev_log_handler = handlers.TimedRotatingFileHandler(
...
proto_sep_dev_log_handler.setFormatter(_dev_formatter)
proto_sep_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

proto_sep_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

proto_sep_usr_log_handler.setLevel(logging.INFO)
...
proto_sep_usr_log_handler.setFormatter(_usr_formatter)
...
proto_sep_console_log_handler = RichHandler(
    level=proto_sep_config.logging.level, rich_tracebacks=True, markup=True, console=console
)
proto_sep_console_log_handler.setFormatter(_console_formatter)
...
def silence_warnings():
    ...
    proto_sep_usr_log_handler.addFilter(warning_filter)
    proto_sep_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    proto_sep_usr_log_handler.removeFilter(warning_filter)
    proto_sep_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    proto_sep_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(proto_sep_dev_log_handler)

    log.addHandler(proto_sep_console_log_handler)

    log.addHandler(proto_sep_usr_log_handler)
